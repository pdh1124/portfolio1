...
<div class="login-page page fix"><!--start login Area-->
	<div class="container">
		<div class="row">
			...
			<div class="col-sm-6">
				<div class="login">
					<form id="findpw-form" method="post" action="/member/findUserPassResult">
						<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}">
						<h2>비밀번호 찾기</h2>
						<p>가입하신 아이디와 이메일을 입력해 주시기 바랍니다.</p>
						<label>아이디<span>*</span></label>
						<input type="text" id="userId" name="userId" placeholder="가입하신 아이디를 입력해주세요" required/>
						<input type="hidden" id="idCheckVal" value="N" />
						<div class="text_fail" id="id-error" style="color:red">아이디를 입력해주세요.</div>
						...
						<input class="findpw-button" type="submit" value="비밀번호 찾기" />
					</form>
				</div>
			</div>
...
$(document).ready(function() {
	...
	/*검사를 통과하지 못했을 경우*/
	var sendForm = $("#findpw-form");
	
	$(".findpw-button").on("click", function(e) {
		e.preventDefault();
